...
from ..jsproxy import Error, JSProxy
...
async def test_proxy_attrs():
    ...
    eq(await proxy.attr, {"name": "attr", "root": "JSProxy"})
    eq(await proxy.foo.bar, {
        "name": "foo",
        "next": {"name": "bar"},
        "root": "JSProxy",
    })
...
async def test_proxy_getitem():
    ...
    eq(await proxy[0], {"name": 0, "root": "JSProxy"})
    eq(await proxy.attr[1], {
        "name": "attr",
        "next": {"name": 1},
        "root": "JSProxy",
    })
...
async def test_proxy_call():
    ...
    eq(await proxy.call(), {"name": "call", "args": [], "root": "JSProxy"})
