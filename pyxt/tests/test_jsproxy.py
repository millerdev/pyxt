...
async def test_proxy_attrs():
    ...
    eq(await get(proxy.attr), {"name": "attr"})
    eq(await get(proxy.foo.bar), {"name": "foo", "next": {"name": "bar"}})
...
async def test_proxy_getitem():
    ...
    eq(await get(proxy[0]), {"name": 0})
    eq(await get(proxy.attr[1]), {"name": "attr", "next": {"name": 1}})
...
async def test_proxy_call():
    ...
    eq(await get(proxy.call()), {"name": "call", "args": []})
